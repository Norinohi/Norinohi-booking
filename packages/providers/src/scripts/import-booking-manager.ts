/**
 * One-off Booking Manager catalogue import.
 *
 * Builds the adapter directly instead of going through `createInventoryProvider`,
 * so the import runs while `PROVIDER_MODE` stays `mock` and the live app keeps
 * quoting against whatever it quotes against today.
 *
 * Imported listings land as `draft` on purpose. Publishing is a separate,
 * deliberate step; see `--publish`.
 *
 *   pnpm --filter @yacht-charter/providers import:booking-manager -- [--publish] [--availability]
 */
...
async function main(): Promise<void> {
  ...
  // The token itself is never printed: this output goes into terminal scrollback
  // and CI logs.
  console.log(`host ${config.baseUrl}  lane ${config.queueKey}`);

  const providerId = await ensureProviderId(db, "booking_manager");
  const provider = new BookingManagerInventoryProvider({ db, config });
