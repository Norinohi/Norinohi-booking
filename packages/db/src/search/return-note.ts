 ...
 *
 * NauSYS writes the rule per base into the listing's own `return_note` text. Booking Manager
 * states it once per company (`checkoutNote`), so it is read off the operator of the offer being
 * sold, and off the listing's operator where no offer is named.
 */
