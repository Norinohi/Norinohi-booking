...
export async function seedSearchWorld(db: TestDatabase["db"]) {
  ...
  await db.insert(provider).values({ id: "prov_ns", code: "nausys", name: "NauSYS" });
}
...
/*
 * One published listing sold by one NauSYS offer, free and carrying a weekly rate across
 * `free`. `rules` omitted means the offer publishes none.
 */
export async function seedListing(
  db: TestDatabase["db"],
  slug: string,
  options: { free: { from: string; to: string }; rules?: Rule[]; weeklyRateMinor?: number },
) {
