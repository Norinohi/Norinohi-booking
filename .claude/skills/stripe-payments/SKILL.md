...
> **Scope split — read this first.** For *generic* Stripe decisions — API choice (we use **PaymentIntents**, not Checkout), API-key/restricted-key handling, webhook signature verification, Tax/Connect, SDK version/migrations — use the vendored **`stripe-best-practices`** skill. This skill is **only** the yacht-charter-specific booking integration layered on top of it. Where they meet (webhooks, idempotency), the official skill has the Stripe mechanics; the rules below add *our* ordering and state transitions.
