...
Authoritative model: `docs/backend-architecture.md` (§1–§2 entities, Appendix A). This skill encodes the _how_. Match these idioms exactly — the existing `packages/db/src/schema/auth.ts` set the precedent.
...
4. **Closed sets = `pgEnum`** (statuses, rule types, `resource_type`, match status). Externalise _user-facing_ labels to `facet_dictionary` for i18n — enums are internal codes.
