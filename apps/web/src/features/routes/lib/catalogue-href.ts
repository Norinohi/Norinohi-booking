 ...
 * the route the visitor was reading is still there when they come back. With a marina from the map, that marina replaces the route's
 * own place, since the visitor has just picked where to start.
 */
export function routeCatalogueHref(route: RouteTarget, marina?: Pick<RouteMarina, "value">) {
  const criteria: SearchCriteria = { duration: String(route.nights) };
  if (route.countryValue) criteria.country = [route.countryValue];
  if (marina) {
    criteria.marina = [marina.value];
  } else {
