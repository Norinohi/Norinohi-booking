...
import type { YachtPrice } from "../lib/discounts";
 ...
 * Contract: `yacht` is the row being edited; null keeps the dialog renderable while closed.
 */
...
export default function PriceDialog({
  ...
  const schema = useMemo(
    () => z.object({ newPrice: z.string().min(1, t("prices.dialog.errors.priceRequired")) }),
    [t],
  );
  ...
  const onSubmit = (values: Values) => {
    /* Persistence lands with the pricing API (the platform-side override of provider
     * prices); until then submitting only acknowledges. */
    void values;
