...
- **Q-MEDIA** _(answered for NauSYS, open for BM)_ NauSYS (Sep 2026): catalogue data, images included, is expected to live on our own server; the cache must be deleted once the contract ends and the data may not be resold, with the rest of the terms in the service contract - so Cloudinary caching and transforming is permitted for NauSYS media. BM states only that data may be displayed "on your website as you wish"; confirm in the BM **T&C** whether caching and transforming is covered before any BM photo gets a `cloudinary_id`.
