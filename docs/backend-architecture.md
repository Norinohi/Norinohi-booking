...
| `operator`                                       | Canonical charter-company identity across providers. **No auth/account** (no owner accounts) — display + attribution only; tax and bank identifiers are dropped before the raw payload is stored, since nothing needs them (§10).                                                                              | M2        |
...
3. **PAYMENT_PENDING** (`checkout.confirm`):
   ...
   - **`capture_method: "manual"` on every charge that runs before the provider has confirmed** (client decision, 2026-08-21: capture only after the operator confirms). The customer authorizes, the card is held, nothing is charged. A balance payment on an already-CONFIRMED booking captures automatically — the charter exists, so there is nothing left to wait for. An authorization lives about seven days (`AUTHORIZATION_TTL_DAYS`); an operator slower than that leaves a lapsed hold, which arrives as `payment_intent.canceled`.
4. **CONFIRMING → CONFIRMED** via **Stripe webhook** (authoritative):
   - `payment_intent.amount_capturable_updated` → `payment.authorized` (with `authorized_at`), enter **CONFIRMING**, call `confirmBooking`. Success → capture the intent, which comes back as `payment_intent.succeeded`. Provider failure → cancel the intent, releasing the hold: nothing was captured, so `PROVIDER_REJECTED` → **REFUND_PENDING** settles straight to **REFUNDED** with no money to send back. A booking that moved under us leaves the hold alone to lapse on its own.
   ...
   - `payment_intent.canceled` → `payment.failed`, and the booking back to `PAYMENT_FAILED` so the customer can authorize again. This is how a hold that expired before the operator answered becomes visible; nothing in our own code fires at that moment.
   - **The endpoint must be subscribed to all eight events we handle**, or the handler for a missing one never runs and the failure is silent: `payment_intent.succeeded`, `payment_intent.amount_capturable_updated`, `payment_intent.canceled`, `payment_intent.processing`, `payment_intent.payment_failed`, `refund.updated`, `charge.dispute.created`, `charge.dispute.closed`.
...
- **Stripe ↔ provider ordering:** the card is **authorized** before the provider commit and **captured** after it, so no money is taken for a charter the operator has not accepted. Which comes first at all is still driven by the hold guarantee: money before provider commit **only when the provider guarantees the option/hold** (`optionExpiryOwnedByProvider`); otherwise hold first, then charge — driven by `capabilities()` (Q-AVAIL, D-PAYORDER). Mock guarantees the hold, so the demo charges then confirms and exercises the provider-rejected → auto-refund branch.
...
- **Q-MEDIA** _(answered, both providers)_ NauSYS (Sep 2026): catalogue data, images included, is expected to live on our own server; the cache must be deleted once the contract ends and the data may not be resold, with the rest of the terms in the service contract. Booking Manager confirmed the same in Sep 2026, verbally - we may cache, copy and transform their media. So mirroring provider photos into our own storage and serving them through our own pipeline is permitted for both. **Not yet in writing for BM**: get the confirmation by email, and the governing T&C clause, before the contract is signed.
...
`provider_raw_payload` is **not** encrypted at rest, so operator financials are kept out of it instead: the catalogue sync drops a company record's VAT and bank identifiers (`vatCode`, `bankAccountNumber` from Booking Manager, `vatcode`, `bankAccounts`, `bankAcounts` from NauSYS) before retention (`withoutOperatorFinancials` in `packages/providers/src/sync/runner.ts`), and migration `0150_strip_operator_financials` removed them from the rows stored earlier. A field of that kind a vendor adds later belongs on that list, not in the raw table.
